<template>
    ...
    <article class="text-base bg-white rounded-lg">
        <div class="flex items-center justify-between">
            <p class="inline-flex items-center mr-3 text-sm text-black font-semibold"><img
                    ...
                    alt="Michael Gough">{{ data.author.name }}</p>
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ timeAgo.format(new Date(data.createdAt*1)) }}</p>
        </div>
        <p class="text-black py-2 ">{{ data.body }}</p>
        <div class="mb-4 flex items-center space-x-4">
            <button
                v-if="replies.length>0"
                @click="toggle_replies"
                type="button"
    ...
    <div v-if="showReplies" class="border-l-4">
        <div class="ml-8" v-for="reply in replies">
