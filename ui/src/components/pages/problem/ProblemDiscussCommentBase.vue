  ...
  <div class="max-w-2xl" :comment_id="data.id">
    ...
    <div v-if="showReplies" class="border-l-4">
      <div class="ml-8 mb-4" v-for="reply in replies">
        <ProblemDiscussCommentBase :data="reply" />
      </div>
    </div>
  </div>
</template>
...
export default {
  name: "ProblemDiscussCommentBase",
  setup(props) {
    ...
    const post_comment = async (e) => {
      ...
      const graphqlQuery = {
        query: `mutation{ createMessage(input: {topic_id: ${topic_id}, author_id: ${user.id}, body: \"${input_comment.value}\", parent_id: \"${parent_id}\"}){id author{name, imgUrl} body createdAt}}`,
      };
    ...
    onMounted(async () => {
      ...
      const graphqlQuery = {
        query: `query{ commentReplies(comment_id: \"${parent_id}\"){author{name, imgUrl} id body createdAt}}`,
      };
      console.log(graphqlQuery.query)
      try {
        const response = await axios.post(endpoint, graphqlQuery, headers);
    ...
    return { replies, post_comment, user, input_comment };
  },
  data() {
    return {
      showReplies: false,
