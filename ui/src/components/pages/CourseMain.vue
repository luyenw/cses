<template>
  <div class="flex flex-row h-[calc(100vh-74px)]">
    ...
    <div class="p-4 w-full overflow-scroll">
      <CourseContent v-if="active_index == 0" :id="Number(id)" />
      <CourseMaterials v-if="active_index == 1" :id="Number(id)" />
      <CoursePages v-if="active_index == 2"/>
      <CourseData v-if="active_index == 3" :id="Number(id)" />
    </div>
  </div>
</template>
<script>
...
import CourseContent from "./course/CourseContent.vue";
